...
use crate::api::{contact, ContactRequest};
...
pub fn component() -> impl IntoView {
    ...
    section().class("px-4 pt-12 pb-24 sm:px-6 lg:px-8").child(
        div().class("mx-auto max-w-5xl").child((
            ...
            p().class("mb-8 max-w-2xl text-xl text-gray-300").child("I work on high-performance, reliable systems with Rust. Of particular interest to me are network and OS projects utilizing cutting-edge technology."),
    ...
    section().class("py-20 px-4 sm:px-6 lg:px-8").child(
        div().class("mx-auto max-w-5xl").child((
            h2().class("mb-12 text-3xl font-bold text-[#ffef5c]").child("whoami"),
            div().class("grid grid-cols-1 gap-12 md:grid-cols-1").child((
                div().class("flex items space-x-6").child((
                    ...
                    div().child((
                        h3().class("mb-1 text-xl font-semibold text-white").child("Alex Thola"),
                        p().class("mb-2 text-gray-300").child("Staff Software Engineer"),
                        a().href("https://www.linkedin.com/in/alexthola/").target("_blank").rel("noopener noreferrer").class("text-sm hover:underline text-[#ffef5c]").child("LinkedIn Profile"),
                    )),
                )),
        ))
    ))),
     section().class("py-20 px-4 sm:px-6 lg:px-8 bg-[#2a2a2a]").child(
         div().class("mx-auto max-w-3xl").child((
             ...
             form().class("space-y-6").on(ev::submit ,move |e| {
                ...
                Show(ShowProps::builder().when(sent).fallback(|| ()).children(ToChildren::to_children(|| p().class("text-[#ffef5c]").child("Message sent successfully! I\'ll get back to you shortly."))).build()
             ),
            )),
         ))
     ),
