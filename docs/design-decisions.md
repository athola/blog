...
- **C — Indieweb Maximal** (blogosphere × fsck fusion). Strongest on simple and
  elegant but weakest on modern: no sans, no dark mode, no newsletter slot made
  it the hardest to evolve and read as deliberately retro for a consulting brand.
...
- **Three webfont families (~200KB)** → Latin subsetting, preload display+body,
  lazy-load mono. (The previous site shipped ~600KB across 28 Poppins files, so
